...
# Every page that shows a number carries this. The README, the PyPI page and /how-fresh already
# said it, but none of those is where a reader lands: a search for "deepgram tts pricing" opens
# /tts/deepgram, which until now showed a rate and no caveat at all. Rendered on the generated
# pages rather than pasted into each, so it cannot be true on some and missing on others.
PRICE_DISCLAIMER = (
    '<Warning>\n'
    '**Reference only, and unofficial.** Voice Prices is a community-maintained catalog. It is '
    'not published by, affiliated with, or endorsed by any vendor listed here, and the rates '
    'below are read off public pricing pages by hand, so they can be out of date or simply '
    'wrong. Vendors change prices without notice, and your own rate may differ with your plan, '
    'region, commitment or negotiated terms.\n\n'
