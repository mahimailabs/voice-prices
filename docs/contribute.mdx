...
A price gets into this catalog one of four ways. On day one they all produce the same number. They differ on day ninety, in how long that number stays correct without a human looking at it.

| | Path | Pick this if | Effort | Correct without a human |
| --- | --- | --- | --- | --- |
| 1 | [Serve a live pricing endpoint](/pricing-feed) | You work at the provider | Half a day, once | Yes, permanently |
| 2 | [Read the provider's pricing API](#2-read-the-providers-pricing-api) | The provider already serves rates as JSON | An importer, in Python | Yes, the weekly job catches drift |
| 3 | [Snapshot a structured rate card](#3-snapshot-a-structured-rate-card) | The provider publishes the whole rate card in one file | A snapshot plus a generator | Until the rate card moves |
| 4 | [Read the pricing page by hand](#4-read-the-pricing-page-by-hand) | Everything else, which is most vendors | About 30 minutes | No, it decays until someone re-checks |
...
The best outcome for everyone, and the only one that does not decay. You publish your current rates at one HTTPS URL, generated from whatever your billing system treats as the source of truth. A scheduled job polls it and the catalog follows your prices without anyone editing this repository again.
