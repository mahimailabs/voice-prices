...
| Requirement | Why |
| --- | --- |
| A stable URL that does not move | It goes in a config file. `/.well-known/pricing.json` on your API host is a good convention. |
| No authentication | So client-side cost calculators can read it too. |
| `Access-Control-Allow-Origin: *` | Same reason. |
| `ETag` or `Last-Modified` | So daily polling costs you a 304 rather than a payload. |
| No query parameters | There is nothing to parameterise. Rates are public and the same for everyone reading them. |
