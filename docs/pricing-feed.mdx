...
Publish one endpoint instead and that stops happening permanently. We poll it every week, and when a rate moves the catalog follows.
...
| `ETag` or `Last-Modified` | So a poll costs you a 304 rather than a payload. |
...
It does not need to be fast or highly available. We poll once a week and treat a timeout as a transient failure, never as a price change.
...
From then on a scheduled job polls your endpoint weekly and flags any rate that has moved away from the catalog.
