...
re-trigger, or open the PR with the screenshot already attached.

If you are looking for a provider to add rather than bringing one, `docs/livekit-coverage.mdx` (generated by `make livekit-coverage`) lists every vendor a LiveKit agent can call and which of them have no rate here. Rows marked "not investigated yet" are unclaimed: nobody has checked whether that vendor publishes a per-unit rate.
